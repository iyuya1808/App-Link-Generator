...
add_action('init', 'applige_register_block');
...
// Initialize Manager
APPLIGE_Manager::init();

// Activation: Schedule Cron
register_activation_hook(__FILE__, function() {
    if (!wp_next_scheduled('applige_daily_update')) {
        wp_schedule_event(time(), 'daily', 'applige_daily_update');
    }
});

// Deactivation: Clear Cron
register_deactivation_hook(__FILE__, function() {
    $timestamp = wp_next_scheduled('applige_daily_update');
    if ($timestamp) {
        wp_unschedule_event($timestamp, 'applige_daily_update');
    }
});

// Register REST API
add_action('rest_api_init', function () {
    register_rest_route('app-link-generator/v1', '/search', array(
        'methods' => 'GET',
    ...
    register_rest_route('app-link-generator/v1', '/lookup', array(
        'methods' => 'GET',
        'permission_callback' => '__return_true',
        'callback' => function ($request) {
